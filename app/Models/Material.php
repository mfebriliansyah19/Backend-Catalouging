...
//     // Filter by user role and name
//     if (!empty($searchQueries->user->role)) {
//         if ($searchQueries->user->role == 1) {
//             // Jika role adalah 1, tidak perlu filter status
//         } elseif ($searchQueries->user->role == 3) {
//             $builder->where('(d_material.status', 'qc', true)
//                     ->orWhere('d_material.status', 'approved', true)
//                     ->groupEnd();
//         }
...
//     // Skip username search for role 1 and 3
//     if (!in_array($searchQueries->user->role, [1, 3]) && !empty($searchQueries->user->username)) {
//         $builder->groupStart()
//                 ->like('qc_user.name', $searchQueries->user->username)
//                 ->orLike('cat_user.name', $searchQueries->user->username)
//                 ->groupEnd();
//     }
...
public function searchMaterialData($page, $perPage, $searchQueries)
{
    ...
    $builder->select('
        d_material.id, 
        d_material.material_number AS materialNumber, 
        d_material.part_number AS partNumber, 
        d_material.raw_data AS rawData, 
        d_material.raw_data2 AS rawData2, 
        d_material.raw_data3 AS rawData3, 
        d_material.raw_data4 AS rawData4, 
        d_material.flag1 AS flag1, 
        d_material.flag2 AS flag2, 
        d_material.result, 
        d_material.attribute_value AS attributeValue, 
        d_material.global_attribute_value AS globalAttributeValue, 
        d_material.inc, 
        d_material.mfr, 
        d_material.group_code AS groupCode, 
        d_material.cat, 
        d_material.qc, 
        d_material.status, 
        d_material.link, 
        d_material.complete_live_description AS completeDesc, 
        d_material.history_complete_desc AS historyDesc, 
        d_material.submitted_at AS submittedAt, 
        d_material.update_qc_at AS qcUpdate, 
        d_material.update_final_at AS finalUpdate, 
        m_inc.inc_name AS incName, 
        d_attribute.attribute_code AS attributeCode, 
        d_attribute.attribute_name AS attributeName, 
        d_attribute.sequence, 
        m_group.group_name AS groupName, 
        qc_user.name AS qcName, 
        qc_user.role_id AS qcRole, 
        cat_user.name AS catName, 
        cat_user.role_id AS catRole
    ')
    ->join('m_inc', 'd_material.inc = m_inc.inc', 'left')
    ->join('d_attribute', 'd_material.inc = d_attribute.inc', 'left')
    ->join('m_group', 'd_material.group_code = m_group.group_code', 'left')
    ...
    if (!empty($searchQueries->user->role)) {
        if ($searchQueries->user->role == '1') {
            ...
            $builder->groupStart();
            $builder->where('d_material.status', 'qc')
                    ->orWhere('d_material.status', 'approved');
            $builder->groupEnd();
        } elseif (in_array($searchQueries->user->role, ['2', '4'])) {
            if (!empty($searchQueries->user->username)) {
                $builder->groupStart();
                if ($searchQueries->user->role == '4') {
                    $builder->where('cat_user.name', $searchQueries->user->username);
                } elseif ($searchQueries->user->role == '2') {
                    $builder->where('cat_user.name', $searchQueries->user->username);
                }
                $builder->groupEnd();
            }
            ...
            switch ($key) {
                case 'searchInc':
                    ...
                    if (strpos($value, '||') !== false) {
                        $subKeywords = explode('||', $value);
                        foreach ($subKeywords as $subKeyword) {
                            $builder->orLike('d_material.inc', trim($subKeyword));
                        }
                    } else {
                        $keywords = explode('|', $value);
                        foreach ($keywords as $keyword) {
                ...
                case 'searchMfr':
                    ...
                    if (strpos($value, '||') !== false) {
                        $subKeywords = explode('||', $value);
                        foreach ($subKeywords as $subKeyword) {
                            $builder->orLike('d_material.mfr', trim($subKeyword));
                        }
                    } else {
                        $keywords = explode('|', $value);
                        foreach ($keywords as $keyword) {
                ...
                case 'searchPartNumber':
                    ...
                    if (strpos($value, '||') !== false) {
                        $subKeywords = explode('||', $value);
                        foreach ($subKeywords as $subKeyword) {
                            $builder->orLike('d_material.part_number', trim($subKeyword));
                        }
                    } else {
                        $keywords = explode('|', $value);
                        foreach ($keywords as $keyword) {
                ...
                case 'searchAfter':
                    ...
                    if (strpos($value, '||') !== false) {
                        $subKeywords = explode('||', $value);
                        foreach ($subKeywords as $subKeyword) {
                            $builder->orLike('d_material.result', trim($subKeyword));
                        }
                    } else {
                        $keywords = explode('|', $value);
                        foreach ($keywords as $keyword) {
                ...
                case 'searchNumber':
                    ...
                    if (strpos($value, '||') !== false) {
                        $subKeywords = explode('||', $value);
                        foreach ($subKeywords as $subKeyword) {
                            $builder->orLike('d_material.material_number', trim($subKeyword));
                        }
                    } else {
                        $keywords = explode('|', $value);
                        foreach ($keywords as $keyword) {
                ...
                case 'searchFlag1':
                    ...
                    if (strpos($value, '||') !== false) {
                        $subKeywords = explode('||', $value);
                        foreach ($subKeywords as $subKeyword) {
                            $builder->orLike('d_material.flag1', trim($subKeyword));
                        }
                    } else {
                        $keywords = explode('|', $value);
                        foreach ($keywords as $keyword) {
                ...
                case 'searchStatus':
                    ...
                    if (strpos($value, '||') !== false) {
                        $subKeywords = explode('||', $value);
                        foreach ($subKeywords as $subKeyword) {
                            $builder->orLike('d_material.status', trim($subKeyword));
                        }
                    } else {
                        $keywords = explode('|', $value);
                        foreach ($keywords as $keyword) {
                ...
                case 'searchBefore':
                    ...
                    if (strpos($value, '||') !== false) {
                        $subKeywords = explode('||', $value);
                        foreach ($subKeywords as $subKeyword) {
                            $builder->orLike('d_material.raw_data', trim($subKeyword));
                        }
                    } else {
                        $keywords = explode('|', $value);
                        foreach ($keywords as $keyword) {
